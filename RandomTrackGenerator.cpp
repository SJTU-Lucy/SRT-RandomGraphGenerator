...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < 36; i++)
    {
        ...
        if (rand() < 0.5 * RAND_MAX)
            current = current - dec > 20 ? current - dec : 20;
        else
            current = current + dec < 40 ? current + dec : 40;
        tmp.x = 40 + current * std::cos(angle);
        tmp.y = 40 + current * std::sin(angle);
        control[i] = tmp;
    }
    ...
    for (int i = 0; i < right.size(); ++i)
        outfile << right[i].x << " " << right[i].y << std::endl;
    outfile.close();            
    // outer-left
    outfile.open(argv[2]);
    for (int i = 0; i < left.size(); ++i)
        outfile << left[i].x << " " << left[i].y << std::endl;
    outfile.close();
    return 0;
}
