...
void createCurve(Point* originPoint, int originCount, 
	std::vector<Point>& curvePoint, std::vector<Point>& left, std::vector<Point>& right) {
	double scale = 0.6;
...
int main(int argc, char* argv[])
{
	...
	for (int i = 0; i < 36; i++)
	{
		...
		Point tmp;
		if (rand() < 0.5 * RAND_MAX) current = current - dec > 10 ? current - dec : 10;
		else current = current + dec < 30 ? current + dec : 30;
